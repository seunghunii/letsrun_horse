...
sapply(1:nrow(tmp),function(l){
  ...
  a <- df_html %>% html_table()
    ...
    sapply(3:length(a),function(m){
      ...
      coll <- data.frame(coll) %>% separate(coll,sep=' ',
      ...
      tmp_sep <- tmp_sep %>% separate(horse_weight,sep='\\(',
      ...
      df_dataa <- left_join(df_dataa,result_df,)
      ...
      sapply(1:nrow(df_dataa),function(j){
        ...
        sqll <- paste0('insert into record_chulmainfo values (',sqll,
