...
#pbsapply(1:length(xppath),function(k){
  ...
  #sapply(1:length(doc_name),function(l){
    ...
    df_record$runway_status <- str_replace_all(df_record$runway_status,
                                               '^$','no_status')
    df_record$horse_num <- na.replace(df_record$horse_num,999)
    df_record$jockey <- str_replace_all(df_record$jockey,'^$',
                                        'unknown')
    df_record$weight <- na.replace(df_record$weight,0)
    df_record$record <- str_replace_all(df_record$record,'^$','unknown')
    ...
    # sapply(1:nrow(df_record),function(j){
      ...
      sqll <- paste0('insert into horseinfo_result_busan values (',sqll,
